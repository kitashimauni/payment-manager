...
`/api/sync/pull?cursor=...` は、同じ認証済みユーザーの変更をサーバー採番の `sync_version` 順で返します。cursorはopaqueなページング値で、論理削除も変更として含まれます。`updatedAt` はクライアント間のLWW判定用に保持し、cursorの進行には使用しません。Pull結果のIndexedDB適用とLast Write Winsは後続段階で実装します。
