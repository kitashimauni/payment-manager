...
- JSON Importはファイル選択後にschemaVersion、必須フィールド、ID、金額、日時、Group・Payment Method参照をランタイム検証し、件数プレビューと明示的な確認操作を要求する。同じIDは`updatedAt`が新しいデータだけを採用し、論理削除も通常更新と同じLWW対象にする。採用したEntityとOutboxは同一IndexedDB readwrite transactionで保存し、途中失敗時に部分適用を残さない。取り込んだローカル変更は既存の同期イベント経由で認証済み同期の対象になる。
