...
- Outboxの追加・削除・同期完了は同一ウィンドウの変更イベントで通知し、NetworkStatusがオンライン状態と同期待ち件数を再取得する。認証済みでオンラインのOutbox追加時は同イベントから同期も開始し、同期中に追加されたローカル変更はsingle-flight完了後に再送する。オンライン復帰直後の一時的な通信失敗は短い遅延で再試行する。
